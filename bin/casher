...
class Casher
  ...
  def run(command, *arguments)
    ...
    cleanup_cache if command == "fetch"
  ...
  def fetch(*urls)
    ...
    cleanup_cache unless archive_found
  end
  ...
  def tar(flag, file, *args, &block)
    ...
    if ! system('sw_vers', [:out, :err] => '/dev/null') and system "sudo -v"
      cmd = "sudo " + cmd
    end
  ...
  def filter_http_params(text)
    text.gsub(/\b(http[^\?]+)\?\S*/, '\1')
  end
end
